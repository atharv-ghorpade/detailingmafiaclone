...
import { Gauge, Droplet, CircleDot, Settings, Snowflake, Zap, Car, Move, Thermometer, Wrench, Sparkles, AlertCircle } from 'lucide-react';
...
export default function App() {
  const [currentPage, setCurrentPage] = useState<'home' | 'about' | 'services' | 'service-detail' | 'experience' | 'blog' | 'contact' | 'booking'>('home');
  const [selectedService, setSelectedService] = useState<string>('Engine Diagnostics');

  const handleNavigate = (page: 'home' | 'about' | 'services' | 'service-detail' | 'experience' | 'blog' | 'contact' | 'booking', serviceTitle?: string) => {
  ...
  useEffect(() => {
    const allowed = ['home','about','services','service-detail','experience','blog','contact','booking'];
  ...
  const currentServiceData = servicesData[selectedService] || servicesData['Engine Diagnostics'];
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {currentPage === 'service-detail' && <ServiceDetailPage onNavigate={handleNavigate} serviceData={currentServiceData} />}
